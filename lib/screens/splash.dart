...
import 'advocate/home_advocate.dart';
...
class _SplashPageState extends State<SplashPage> with TickerProviderStateMixin {
      ...
      _textController.addStatusListener((status) {
        if (status == AnimationStatus.completed) {
          Future.delayed(const Duration(seconds: 3), () async {
            await _navigateBasedOnLogin(context);
          });
        }
      });
    });
  ...
  /// ✅ New method: Checks login status and routes accordingly
  Future<void> _navigateBasedOnLogin(BuildContext context) async {
    ...
    if (isLoggedIn && userRole != null) {
      if (userRole == 'Client') {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (_) => const HomeClientScreen(userName: "Client"),
          ),
        );
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (_) => const HomeAdvocateScreen(userName: "Advocate"),
          ),
        );
      }
    } else {
        ...
        child: Column(
          ...
          children: [
            AnimatedBuilder(
              ...
              child: Image.asset(
                'assets/logo.png', // ✅ updated path to match assets/icon/
                width: logoMaxSize,
                height: logoMaxSize,
              ),
            ),
